...
class TaskSchedule {
  ...
  CivilDay get startDate {
    if (schedules.isEmpty) {
      return CivilDay.fromDateTime(DateTime.now());
    }
    final index =
  ...
  factory TaskSchedule.fromMap(
    Map<String, dynamic> data, {
    String? id,
    bool hasPendingWrites = false,
    bool isFromCache = false,
  }) {
    final resolvedId = id ?? data['id'] as String? ?? TaskSchedule.generateId();
  ...
  factory TaskSchedule.fromFirestore(dynamic snapshot, [dynamic options]) {
    final data = (snapshot as dynamic).data() as Map<String, dynamic>?;
    if (data == null) {
      throw Exception('Data is null for document ${snapshot.id}');
    }
    final metadata = (snapshot as dynamic).metadata;
    final hasPendingWrites = metadata?.hasPendingWrites as bool? ?? false;
    final isFromCache = metadata?.isFromCache as bool? ?? false;
  ...
  TaskModification edit({
    ...
    final oldSchedulesJson =
        schedules.map((s) => s.toJson()).toList().toString();
    final newSchedulesJson =
        resolvedSchedules.map((s) => s.toJson()).toList().toString();
    if (oldSchedulesJson != newSchedulesJson) {
      ...
      if (resolvedSchedules.isNotEmpty) {
        changes['schedulingPolicy'] =
            resolvedSchedules.first.schedulingPolicy.toJson();
        changes['missedOccurrencePolicy'] =
            resolvedSchedules.first.missedOccurrencePolicy.toJson();
  ...
  TaskSchedule _copyWith({
    ...
    return TaskSchedule(
      ...
      appLaunchUrl:
          clearAppLaunchUrl ? null : (appLaunchUrl ?? this.appLaunchUrl),
      workflowType:
