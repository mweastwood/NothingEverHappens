...
class TaskInstance {
  ...
  factory TaskInstance.fromMap(
    ...
    final completedByUserIds =
        completedByUserIdsRaw.map((e) => e.toString()).toList();

    final completedAt = _parseDateTime(data['completedAt']);
  ...
  factory TaskInstance.fromFirestore(dynamic snapshot, [dynamic options]) {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      if (notificationRelativeTimes.isNotEmpty)
        'notificationRelativeTimes':
            notificationRelativeTimes.map((t) => t.toJson()).toList(),
      'isFamily': isFamily,
  ...
  TaskInstance copyWith({
    ...
    return TaskInstance(
      ...
      statusReason:
          clearStatusReason ? null : (statusReason ?? this.statusReason),
