...
TaskScheduleRule convertRuleToKind(
  ...
  if (kind != HierarchicalRecurrenceKind.oneOff) {
    final currentNow = now ?? CoreClock.now;
    final tomorrow =
        CivilDay.fromDateTime(currentNow.add(const Duration(days: 1)));
    if (existingRule is OneOffSchedule &&
        scheduledDate == tomorrow &&
        startRelativeTime.dayOffset == -1) {
      scheduledDate = CivilDay.fromDateTime(currentNow);
      startRelativeTime = RelativeTime(
        dayOffset: 0,
        hour: startRelativeTime.hour,
        minute: startRelativeTime.minute,
      );
    }
  }
    ...
    case HierarchicalRecurrenceKind.dailyCompletionRelative:
      return DailySchedule(
        ...
        schedulingPolicy: CompletionRelativePolicy(
          ...
          targetHour: startRelativeTime.hour,
          targetMinute: startRelativeTime.minute,
        ),
        missedOccurrencePolicy: missedOccurrencePolicy,
      );
