...
void main() {
  group('Multiple Schedules & Standardized Timing Unit Tests', () {
    ...
    test('Migration maps legacy schedule and dailyTimes correctly', () {
      // Simulate firestore load of a legacy task with schedule & dailyTimes
      final legacyDocJson = {
        'title': 'Legacy task',
        ...
        'dueRelativeTime': {'dayOffset': 0, 'hour': 17, 'minute': 0},
        'dailyTimes': [
          {
            'startHour': 10,
            'startMinute': 0,
          ...
          {'startHour': 14, 'startMinute': 0, 'dueHour': 16, 'dueMinute': 0},
        ],
      };

      // Construct using constructor migration helper
      final task = Task(
        id: 'legacy-task',
        title: legacyDocJson['title'] as String,
        description: legacyDocJson['description'] as String,
        schedule: TaskSchedule.fromJson(
          legacyDocJson['schedule'] as Map<String, dynamic>,
        ),
        startRelativeTime: RelativeTime.fromJson(
          legacyDocJson['startRelativeTime'] as Map<String, dynamic>,
        ),
        dueRelativeTime: RelativeTime.fromJson(
          legacyDocJson['dueRelativeTime'] as Map<String, dynamic>,
        ),
        dailyTimes: (legacyDocJson['dailyTimes'] as List<dynamic>)
            .map(
              (dt) => DailyOccurrenceTime.fromJson(dt as Map<String, dynamic>),
            )
            .toList(),
      );

      // Verify it migrated dailyTimes into separate schedules
      expect(task.schedules.length, 2);

      final first = task.schedules[0] as DailySchedule;
      expect(first.interval, 2);
      expect(
        first.startRelativeTime.time,
        const TimeOfDay(hour: 10, minute: 0),
      );
      expect(first.dueRelativeTime.time, const TimeOfDay(hour: 12, minute: 0));
      expect(
        first.notificationRelativeTime?.time,
        const TimeOfDay(hour: 9, minute: 30),
      );

      final second = task.schedules[1] as DailySchedule;
      expect(second.interval, 2);
      expect(
        second.startRelativeTime.time,
        const TimeOfDay(hour: 14, minute: 0),
      );
      expect(second.dueRelativeTime.time, const TimeOfDay(hour: 16, minute: 0));
      expect(second.notificationRelativeTime, isNull);
    });
