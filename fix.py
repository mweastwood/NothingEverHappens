import os
with open('app/test/screens/task_list_screen_test.dart', 'r') as f:
    text = f.read()

text = text.replace("""    tasksSubject = BehaviorSubject<List<TaskSchedule>>(sync: true)
      ..add(initialTasks);
    ...
    AppClock.timeNotifier.addListener(clockListener!);""", """    final initialInstances = [
      TaskInstance(
        id: 'I-1_2024-01-01',
        scheduleId: '1',
...
text = text.replace("""    when(mockTaskRepository.addTaskSchedule(any)).thenAnswer((
      invocation,
    ) async {
...
text = text.replace("""      tasksSubject.add([recurringTask]);

      when(""", """      tasksSubject.add([recurringTask]);
      instancesSubject.add([
        TaskInstance(
...
text = text.replace("""        tasksSubject.add([advancedTask]);
        return null;""", """        tasksSubject.add([advancedTask]);
        instancesSubject.add([]);
        return null;""")

text = text.replace("""      tasksSubject.add([todayTask, tomorrowTask]);

      await tester.pumpWidget(createScreen());""", """      tasksSubject.add([todayTask, tomorrowTask]);
      instancesSubject.add([
        TaskInstance(
...
text = text.replace("""      tasksSubject.add([activeOneOffTask]);

      await tester.pumpWidget(createScreen());""", """      tasksSubject.add([activeOneOffTask]);
      instancesSubject.add([
        TaskInstance(
...
text = text.replace("""      tasksSubject.add([snoozedOneOffTask]);

      await tester.pumpWidget(createScreen());""", """      tasksSubject.add([snoozedOneOffTask]);
      instancesSubject.add([
        TaskInstance(
...
text = text.replace("""    tasksSubject.add([futureTodayTask]);

    await tester.pumpWidget(createScreen());""", """    tasksSubject.add([futureTodayTask]);
    instancesSubject.add([
      TaskInstance(
...
text = text.replace("""      tasksSubject.add([task1, task2]);
...
with open('app/test/screens/task_list_screen_test.dart', 'w') as f:
    f.write(text)
print('Done py')
